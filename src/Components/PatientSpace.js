...
// ─── Mock Data ───────────────────────────────────────────────────────────────
const MOCK_VITALS = [
  { date: "20/04", bpm: 72, sys: 118, dia: 76, temp: 36.7 },
  { date: "21/04", bpm: 75, sys: 121, dia: 79, temp: 36.8 },
  { date: "22/04", bpm: 68, sys: 115, dia: 74, temp: 36.6 },
  { date: "23/04", bpm: 80, sys: 128, dia: 83, temp: 37.1 },
  { date: "24/04", bpm: 74, sys: 119, dia: 77, temp: 36.9 },
  { date: "25/04", bpm: 71, sys: 116, dia: 75, temp: 36.7 },
  { date: "26/04", bpm: 73, sys: 120, dia: 78, temp: 36.8 },
];

const MOCK_BIO = [
  { date: "15/04", poids: 74.2, imc: 24.1 },
  { date: "18/04", poids: 73.8, imc: 23.9 },
  { date: "22/04", poids: 73.5, imc: 23.8 },
  { date: "26/04", poids: 73.1, imc: 23.7 },
];

const MOCK_ACTIVITY = [
  { date: "20/04", pas: 7800, dist: 5.8, cal: 312 },
  { date: "21/04", pas: 10200, dist: 7.6, cal: 408 },
  { date: "22/04", pas: 6400, dist: 4.8, cal: 256 },
  { date: "23/04", pas: 9100, dist: 6.8, cal: 364 },
  { date: "24/04", pas: 11500, dist: 8.6, cal: 460 },
  { date: "25/04", pas: 8300, dist: 6.2, cal: 332 },
  { date: "26/04", pas: 9800, dist: 7.3, cal: 392 },
];

const MOCK_NUTRI = [
  { date: "24/04", cal: 1950, qualite: "Bonne" },
  { date: "25/04", cal: 2100, qualite: "Moyenne" },
  { date: "26/04", cal: 1820, qualite: "Excellente" },
];

const MOCK_PATHO = [
  { date: "24/04", glycemie: 5.2, spo2: 98 },
  { date: "25/04", glycemie: 5.5, spo2: 97 },
  { date: "26/04", glycemie: 4.9, spo2: 99 },
];

const MOCK_GOALS = [
  { id: 1, type: "Pas quotidiens", cible: 10000, actuel: 9800, unite: "pas", periode: "Quotidien", icon: "🦶" },
  { id: 2, type: "Eau", cible: 2.5, actuel: 1.8, unite: "L", periode: "Quotidien", icon: "💧" },
  { id: 3, type: "Sommeil", cible: 8, actuel: 7.2, unite: "h", periode: "Quotidien", icon: "🌙" },
  { id: 4, type: "Calories brûlées", cible: 400, actuel: 392, unite: "kcal", periode: "Quotidien", icon: "🔥" },
];
...
function BarChart({ data, valueKey, color, max, label }) {
  const maxVal = max || Math.max(...data.map(d => d[valueKey])) * 1.1;
  return (
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        <Card>
          ...
          <BarChart data={activity} valueKey="pas" color={C.primary} label="pas" />
        </Card>
        <Card>
          <div style={{ fontSize: 14, fontWeight: 600, color: C.primaryDark, marginBottom: 14 }}>Fréquence cardiaque — 7 jours</div>
          <BarChart data={vitals} valueKey="bpm" color="#E74C3C" label="bpm" />
        </Card>
      </div>

      {/* Weekly summary + Goals preview */}
...
function VitalsPage({ vitals, setVitals }) {
  ...
  const save = () => {
    ...
    const today = new Date().toLocaleDateString("fr", { day: "2-digit", month: "2-digit" });
    setVitals(v => [...v, { date: today, bpm: +form.bpm, sys: +form.sys, dia: +form.dia, temp: +form.temp }]);
    setModal(false);
    setSaved(true);
...
function BiometricsPage({ bio, setBio }) {
  ...
  const save = () => {
    if (!poids) return;
    const today = new Date().toLocaleDateString("fr", { day: "2-digit", month: "2-digit" });
    setBio(b => [...b, { date: today, poids: +poids, imc: +calcIMC(+poids) }]);
    setModal(false);
    setSaved(true);
    setTimeout(() => setSaved(false), 3000);
    setPoids("");
...
function ActivityPage({ activity, setActivity }) {
  ...
  const save = () => {
    if (!form.pas) return;
    const today = new Date().toLocaleDateString("fr", { day: "2-digit", month: "2-digit" });
    setActivity(a => [...a, { date: today, pas: +form.pas, dist: +form.dist, cal: +form.cal }]);
    setModal(false);
    setSaved(true);
    setTimeout(() => setSaved(false), 3000);
...
function NutritionPage({ nutri, setNutri }) {
  ...
  const save = () => {
    if (!form.cal) return;
    const today = new Date().toLocaleDateString("fr", { day: "2-digit", month: "2-digit" });
    setNutri(n => [...n, { date: today, cal: +form.cal, qualite: form.qualite }]);
    setModal(false);
    setSaved(true);
    setTimeout(() => setSaved(false), 3000);
  ...
  const last = nutri[nutri.length - 1];
...
function PathologyPage({ patho, setPatho }) {
  ...
  const save = () => {
    ...
    const today = new Date().toLocaleDateString("fr", { day: "2-digit", month: "2-digit" });
    setPatho(p => [...p, { date: today, glycemie: +form.glycemie, spo2: +form.spo2 }]);
    setModal(false);
    setSaved(true);
    setTimeout(() => setSaved(false), 3000);
  ...
  const last = patho[patho.length - 1];
  const glyAlert = last.glycemie > 7 || last.glycemie < 3.9;
...
function GoalsPage({ goals, setGoals }) {
  ...
  const save = () => {
    ...
    if (editGoal) {
      setGoals(gs => gs.map(g => g.id === editGoal.id ? { ...g, ...form, cible: +form.cible } : g));
    } else {
      setGoals(gs => [...gs, { id: Date.now(), ...form, cible: +form.cible, actuel: 0 }]);
    }
    setModal(false);
  };
...
export default function PatientSpace() {
  ...
  const [nutri, setNutri] = useState(MOCK_NUTRI);
  const [patho, setPatho] = useState(MOCK_PATHO);
